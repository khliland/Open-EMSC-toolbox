 function [EMSCModelFinal]=make_emsc_rep_mod_progress(ZFTIRSel,SamplesNameStart,SamplesNameEnd,EMSCOption,NComp,EMSCWeights,plotid,h)
...
g=create_group2(ZFTIRSel,SamplesNameStart,SamplesNameEnd);
i1=find(g.g.d==1);
...
waitbar(0,h,['Creating replicate model (' num2str(0) '%)']);

ZSaisir.v = str2num(ZSaisir.v);
upd = round(1:(N-1)/99:N);
...
for i=1:N
    ...
    if ism
        waitbar(i,h,['Creating replicate model (' num2str(prc) '%)']);
    end
