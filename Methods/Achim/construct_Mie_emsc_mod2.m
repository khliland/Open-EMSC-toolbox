function [EMSCMieMod]=construct_Mie_emsc_mod2(ZRef,MieOpt,BaseMod,NCOMP,NCOMPREF,AlphaMin,AlphaMax,NSpacingsAlpha,ZWeightsRef);
...
EMSCMieMod = BaseMod;
for i=1:size(EMSCMieMod.ModelSpecNames,1)
    if ~isempty(strfind(EMSCMieMod.ModelSpecNames(i,:),'Reference'))
        EMSCMieMod.Model(:,i) = NewRefSpec';
    end
end


% if (BaseMod==1)     % MSC model
%     [EMSCMieMod]=make_emsc_modfunc(ZRef,3);
%     EMSCMieMod.Model(:,2:2)=NewRefSpec';
% elseif (BaseMod==2)     % Physical EMSC model
